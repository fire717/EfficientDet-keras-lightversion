...
class PascalVocGenerator(Generator):
    ...
    def __parse_annotation(self, element_all):
        """
        Parse an annotation given an XML element.
        """

        size = _findNode(element_all, 'size')
        img_w = _findNode(size, 'width', 'size.width', parse=float)
        img_h = _findNode(size, 'height', 'size.height', parse=float)

        element = _findNode(element_all, 'object')
    ...
    def __parse_annotations(self, xml_root):
        ...
        annotations = {'labels': np.empty((0,), dtype=np.int32),
                       'bboxes': np.empty((0, 4))}
        for i, element in enumerate(xml_root.iter('annotation')):
            try:
